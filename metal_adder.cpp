...
const unsigned int vector_length = 1 << 25;
...
void metal_adder::zero_buffer(MTL::Buffer *pBuffer) {
  auto *data_ptr = (float *)pBuffer->contents();
  for (unsigned long index = 0; index < vector_length; index++) {
    data_ptr[index] = 0.0;
  }
}

void metal_adder::encode_add_command(
  ...
  NS::UInteger _thread_group_size =
      _addFunctionPSO->maxTotalThreadsPerThreadgroup();
...
void metal_adder::init_with_device(MTL::Device *device) {
  ...
  auto function = NS::String::string("add", NS::ASCIIStringEncoding);
...
void metal_adder::send_compute_command() {
  auto command_buffer = _CommandQueue->commandBuffer();
...
unsigned int metal_adder::get_size() { return vector_length; }
